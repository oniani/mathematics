...
Since we got that n = 76.93, we should round it up (as the sample size cannot
be a floating point number) and we get n = 77. Hence, the answer is that the
sample size is 77.
...
interaction between the variables. The least degree of interaction seems to
be between variables height and fert. However, further analysis will show the
...
```{r}
emmeans(fit2, pairwise ~ height | interval)
emmeans(fit2, pairwise ~ fert   | interval)
```
